...
public class PlaceAsyncSaveService {
    ...
    public void savePlacesAsync(List<Place> places) {
        logger.info("PLACE ASYNC SAVE SERVICE - ASYNC SAVE START - Places count: {}", places.size());
        ...
        for (Place place : places) {
            try {
                ...
                savedCount++;
                logger.info("PLACE ASYNC SAVE SERVICE - PLACE SAVED - Place: {} (ID: {})", place.getName(), place.getGooglePlaceId());
            } catch (PlaceAlreadyExistsException e) {
                skippedCount++;
                logger.debug("PLACE ASYNC SAVE SERVICE - PLACE ALREADY EXISTS - Place: {} (ID: {})", place.getName(), place.getGooglePlaceId());
            } catch (Exception e) {
                errorCount++;
                logger.error("PLACE ASYNC SAVE SERVICE - ERROR SAVING PLACE - Place: {}, ID: {}, Error: {}",
                        place.getName(), place.getGooglePlaceId(), e.getMessage(), e);
            }
        }

        logger.info("PLACE ASYNC SAVE SERVICE - ASYNC SAVE FINISH - Total: {}, Saved: {}, Skipped: {}, Errors: {}",
                places.size(), savedCount, skippedCount, errorCount);
    }
    ...
    protected void savePlaceIndividually(Place place) {
        ...
        if (existingPlace.isEmpty()) {
            ...
            boolean needsDefaults = place.getContact() == null || place.getContact().isEmpty()
            ...
            if (needsDefaults) {
                place = Place.builder()
                        ...
                        .contact(place.getContact() != null && !place.getContact().isEmpty() ? place.getContact() : "")
                        .address(place.getAddress() != null ? place.getAddress() : "")
                        .city(place.getCity())
                        .rating(place.getRating() != null ? place.getRating() : 0.0f)
                        .userRatingsTotal(place.getUserRatingsTotal() != null ? place.getUserRatingsTotal() : 0)
                        .principalPhoto(place.getPrincipalPhoto() != null ? place.getPrincipalPhoto() : "")
                        ...
                        .distanceOfLocal(place.getDistanceOfLocal() != null ? place.getDistanceOfLocal() : 0.0f)
