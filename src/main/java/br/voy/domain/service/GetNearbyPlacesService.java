...
public class GetNearbyPlacesService implements GetNearbyPlacesUseCase {
    ...
    public NearbyPlaces getNearbyPlaces(
        ...
        logger.info("GET NEARBY PLACES SERVICE - GET NEARBY PLACES START - Coordinates: {}, Radius: {}, PlaceType: {}, NextPageToken: {}",
                coordinates, radius, placeType, nextPageToken);

        // Decode pagination state
        PaginationTokenEncoder.PaginationState paginationState;
        try {
            paginationState = nextPageToken != null && !nextPageToken.isEmpty()
        ...
        logger.info("GET NEARBY PLACES SERVICE - Pagination State - Offset: {}, Shown IDs: {}, Google Token: {}",
                offset, shownPlaceIds.size(), googleToken != null ? "present" : "null");

        // Fetch all database places (we'll paginate from this list)
        List<Place> allDbPlaces = placeRepository.findNearbyPlacesByCoordinates(
        ...
        if (!allDbPlaces.isEmpty() && offset < allDbPlaces.size()) {
            logger.info("GET NEARBY PLACES SERVICE - Fetching from database starting at offset {}", offset);

            List<Place> dbPagePlaces = allDbPlaces.stream()
                    .skip(offset)
            ...
            logger.info("GET NEARBY PLACES SERVICE - Got {} places from database (after deduplication)", dbPagePlaces.size());
            ...
            if (resultPlaces.size() >= pageSize) {
                ...
                String nextToken = PaginationTokenEncoder.encode(nextOffset, newShownPlaceIds, null);

                logger.info("GET NEARBY PLACES SERVICE - Returning full page from database. Next offset: {}", nextOffset);
            ...
            if (offset + dbPagePlaces.size() < allDbPlaces.size()) {
                ...
                logger.info("GET NEARBY PLACES SERVICE - Returning {} places from database (partial page). Next offset: {}",
                        resultPlaces.size(), nextOffset);
        ...
        if (placesNeeded > 0) {
            logger.info("GET NEARBY PLACES SERVICE - Database exhausted or insufficient. Need {} more places. Fetching from Google API", placesNeeded);

            // Get all database place IDs to filter duplicates
            Set<String> allDbPlaceIds = allDbPlaces.stream()
            ...
            NearbyPlaces googleResponse = googlePlacesPort.getNearbyPlaces(
                    coordinates,
                    radius,
                    placeType,
                    googleToken
            ...
            logger.info("GET NEARBY PLACES SERVICE - Added {} places from Google API (filtered {} duplicates)",
            ...
            if (googleResponse.getNextTokenPage() != null && !googleResponse.getNextTokenPage().isEmpty()) {
                // Continue with Google API pagination, database is exhausted
                nextToken = PaginationTokenEncoder.encode(allDbPlaces.size(), newShownPlaceIds, googleResponse.getNextTokenPage());
                logger.info("GET NEARBY PLACES SERVICE - Google has more results. Next token includes Google pagination token");
            ...
            logger.info("GET NEARBY PLACES SERVICE - GET NEARBY PLACES FINISH - Returning {} places total ({} from DB + {} from Google)",
                    resultPlaces.size(), resultPlaces.size() - filteredGooglePlaces.size(), filteredGooglePlaces.size());
