...
public interface PlaceJpaRepository extends JpaRepository<PlaceModel, Long> {
    ...
    @Query(value = "SELECT * FROM registration.place p " +
            ...
            nativeQuery = true)
    List<PlaceModel> findNearbyPlacesByCoordinates(@Param("latitude") double latitude,
                                                    @Param("longitude") double longitude,
                                                    @Param("radiusInMeters") int radiusInMeters);

    @Query("SELECT p FROM PlaceModel p WHERE (p.latitude = 0 OR p.latitude IS NULL) AND (p.longitude = 0 OR p.longitude IS NULL) AND p.googlePlaceId IS NOT NULL")
