...
class PlaceNearbyPersistenceIntegrationTest {
    ...
    private static final String BASE_URL_TEMPLATE = "http://localhost:%d%s?latitude=-29.38&longitude=-50.87";
    ...
    @MockBean
    private GetRecommendedPlacesUseCase placeRecommendationUseCase;
    ...
    void shouldPersistPlacesReturnedByGoogleApiToDatabase() {
        ...
        await()
                ...
                .untilAsserted(() -> {
                    assertThat(placeJpaRepository.findByGooglePlaceId("persist-test-id-001")).isPresent();
                    assertThat(placeJpaRepository.findByGooglePlaceId("persist-test-id-002")).isPresent();
                });
    ...
    void shouldNotCreateDuplicateEntryWhenSameGooglePlaceIsReceivedAgain() {
        ...
        await()
                .atMost(10, TimeUnit.SECONDS)
                .pollInterval(Duration.ofMillis(500))
                .until(() -> placeJpaRepository.findByGooglePlaceId("persist-test-dup-001").isPresent());
        ...
        await()
                ...
                .until(() -> placeJpaRepository.findAll().stream()
                        .filter(p -> "persist-test-dup-001".equals(p.getGooglePlaceId()))
                        .count() == 1L);
    }
    ...
    void shouldReturnPlacesFromDatabaseWithoutCallingGoogleApi() {
        placeJpaRepository.saveAll(List.of(
                PlaceModel.builder().googlePlaceId("db-src-001").name("Place One").city("Test City").latitude(-29.382).longitude(-50.872).build(),
                PlaceModel.builder().googlePlaceId("db-src-002").name("Place Two").city("Test City").latitude(-29.383).longitude(-50.873).build(),
                PlaceModel.builder().googlePlaceId("db-src-003").name("Place Three").city("Test City").latitude(-29.384).longitude(-50.874).build(),
                PlaceModel.builder().googlePlaceId("db-src-004").name("Place Four").city("Test City").latitude(-29.385).longitude(-50.875).build(),
                PlaceModel.builder().googlePlaceId("db-src-005").name("Place Five").city("Test City").latitude(-29.386).longitude(-50.876).build()
        ));
    ...
    void shouldFallBackToGoogleApiWhenDatabasePlacesAreExhaustedByNextToken() {
        placeJpaRepository.saveAll(List.of(
                PlaceModel.builder().googlePlaceId("db-page-001").name("Place One").city("Test City").latitude(-29.382).longitude(-50.872).build(),
                PlaceModel.builder().googlePlaceId("db-page-002").name("Place Two").city("Test City").latitude(-29.383).longitude(-50.873).build(),
                PlaceModel.builder().googlePlaceId("db-page-003").name("Place Three").city("Test City").latitude(-29.384).longitude(-50.874).build(),
                PlaceModel.builder().googlePlaceId("db-page-004").name("Place Four").city("Test City").latitude(-29.385).longitude(-50.875).build(),
                PlaceModel.builder().googlePlaceId("db-page-005").name("Place Five").city("Test City").latitude(-29.386).longitude(-50.876).build()
        ));

        var googlePlace = Place.builder()
        ...
        var secondResponse = restTemplate.getForEntity(buildUrl() + "&nextPageToken=" + nextToken, NearbyPlacesResponse.class);
